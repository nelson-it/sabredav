...
class PDO extends \Sabre\CardDAV\Backend\PDO {
    ...
    function __construct ($addressBooksTableName = 'addressbooks', $cardsTableName = 'cards', $addressBookChangesTableName = 'addressbookchanges') {
    ...
    function getMultipleCards ($addressBookId, array $uris) {
        ...
        if (count($result) == 0) {
            $query = 'SELECT t0.id, t1.uri, t0.lastmodified, t0.etag, t0.size FROM ' . $this->cardsTableName .
                     ' t0 INNER JOIN mne_sabredav.uri2personid t1 ON t0.id = t1.personid WHERE addressbookid = ? AND t1.uri IN (';
            // Inserting a whole bunch of question marks
            $query .= implode(',', array_fill(0, count($uris), '?'));
            $query .= ')';
